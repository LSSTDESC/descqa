//**
// all instances of the name 'foldersTree' in the original script
// became 'fld0' and all instances of 'fld0' now present were
// previously 'foldersTree'. The name change makes the automatic
// generation of 'infoNodes.js' in 'home.py' easier.
//
// I marked all my other changes with '-nttaylor'
//**
...
// ***************************************************************** 
function Folder(folderDescription, hreference, path) //constructor 
{ 
  //constant data 
  this.desc = folderDescription; 
  this.hreference = hreference;
  this.path = path  // my addition -nttaylor
  this.id = -1;
  this.navObj = 0;
  this.iconImg = 0; 
  this.nodeImg = 0;
...
{ 
  ...
  if (this.getID()!=fld0.getID() && PRESERVESTATE && !this.isOpen) //closing
  {
...
{ 
  //**
  // The following lines automatically uncheck the checkbox
  // next to a folder which the user has just closed.
  // 'this.navObj' is the folder as it is known to the
  // 'document' object. Its first child, first grandchild,
  // and first great-grandchild are <table>, <tbody>, and
  // <tr> elements respectively.    - nttaylor
  //**
  var tr = this.navObj.firstChild.firstChild.firstChild;
  ...
  for (var i=0; i < tr.childNodes.length; i++) {
    if (tr.childNodes[i].firstChild != null) {
      if (tr.childNodes[i].firstChild.nodeName == "INPUT") {
        tr.childNodes[i].firstChild.checked = false;
      }
    }
  }
  // end of my addition   -nttaylor
...
function blockStartHTML(idprefix) {
  ...
  if (browserVersion == 2) 
    ...
    //**
    // I commented out the below line because it contains an invalid value
    // for "position", namely "block". I'm not sure what the author intended.
    // It must definitely be either "static" or "relative" but I can't see
    // any reason for setting this at all, so I removed it completely from
    // my modified version of the line. Incidentally, this means that the
    // div's "position" property will assume its default value of "static".
    //                                        -nttaylor
    // **
...
{ 
  ...
  {
    alert("Treeview was not able to find the node object corresponding to ID=" + folderId + ". If the configuration file sets a.xID values, it must set them for ALL nodes, including the fld0 root.")
    return;
  }
...
{ 
  ...
  {
    alert("Treeview was not able to find the node object corresponding to ID=" + folderId + ". If the configuration file sets a.xID, it must set fld0.xID as well.")
    return;
  }
...
{ 
  ...
  if (folderObj.id!=fld0.id && PRESERVESTATE)
  {
    currentOpen = GetCookie("clickedFolder")
    if (currentOpen == null)
      currentOpen = ""
...
function gFld(description, hreference, path) 
{ 
  // I added 'path' to this function's input parameters
  // and to the constructor for class Folder  -nttaylor
  folder = new Folder(description, hreference, path);
  return folder;
} 
...
function storeAllNodesInClickCookie(treeNodeObj)
{
  ...
  if (typeof treeNodeObj.setState != "undefined") //is folder
  {
    ...
    if (treeNodeObj.getID() != fld0.getID())
...
function CookieBranding(name) {
  if (typeof fld0.treeID != "undefined")
    return name+fld0.treeID //needed for multi-tree sites. make sure treeId does not contain cookieCutter
  else
    return name
}
...
{ 
  ...
  //fld0 (with the site's data) is created in an external .js (demoFramesetNode.js, for example)
  fld0.initialize(0, true, "") 
  if (supportsDeferral && !STARTALLOPEN) {
    fld0.renderOb(null) //delay construction of nodes
  }

  else {
    renderAllTree(fld0, null);

    if (PRESERVESTATE && STARTALLOPEN)
      storeAllNodesInClickCookie(fld0)
