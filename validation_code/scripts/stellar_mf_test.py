...
def read_obs_mf(fn, usecols):
    binctr, mhist, merr = np.loadtxt(fn, unpack=True, usecols=usecols)
...
mp.step(sbinctr, shist, where="mid", label=sim.name)
mp.errorbar(obinctr, ohist, yerr=[ohmin-ohist, ohmax-ohist], label=obs.name, fmt='o')
